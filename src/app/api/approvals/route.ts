...
import { approvalDashboard } from '@/lib/approvalDashboard'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const chainIds = chainIdsParam
      ...
      : [1, 8453, 56, 42161, 137]
    const result = await approvalDashboard.getApprovals(address, chainIds)
    return NextResponse.json(result)
  } catch (err) {
    return NextResponse.json({ error: sanitizeErrorMessage(err) || 'Approval scan failed' }, { status: 500 })
  }
}
