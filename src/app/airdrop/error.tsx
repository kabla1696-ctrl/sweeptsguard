...
export default function AirdropError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    fetch('/api/sentry', {
      ...
      body: JSON.stringify({ message: error.message, level: 'error', context: { page: 'airdrop', digest: error.digest } }),
    }).catch(() => {})
  }, [error])
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#050507] via-[#0a0a0f] to-[#050507] text-white flex items-center justify-center">
      <div className="max-w-md mx-auto text-center px-6 animate-[fade-in_0.6s_ease-out]">
        ...
        <h1 className="text-2xl font-bold mb-3 text-red-400">Airdrop Error</h1>
        <p className="text-gray-400 text-sm mb-8 leading-relaxed">Failed to load airdrop data. This has been logged.</p>
