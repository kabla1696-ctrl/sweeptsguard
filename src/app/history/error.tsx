...
export default function HistoryError({
  ...
  useEffect(() => {
    fetch('/api/sentry', {
      ...
      body: JSON.stringify({ message: error.message, level: 'error', context: { page: 'history', digest: error.digest } }),
    }).catch(() => {})
  }, [error])
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#050507] via-[#0a0a0f] to-[#050507] text-white flex items-center justify-center">
      <div className="max-w-md mx-auto text-center px-6 animate-[fade-in_0.6s_ease-out]">
        ...
        <h1 className="text-2xl font-bold mb-3 text-red-400">History Error</h1>
        <p className="text-gray-400 text-sm mb-8 leading-relaxed">Failed to load scan history. This has been logged.</p>
        <div className="flex gap-3 justify-center flex-wrap">
          ...
          <a href="/scan" className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-gray-300 hover:bg-white/10 transition-all duration-300">New Scan</a>
        </div>
      </div>
    </main>
  )
