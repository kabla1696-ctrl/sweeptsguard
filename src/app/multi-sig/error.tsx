  ...
  useEffect(() => {
    fetch('/api/sentry', {
      ...
      body: JSON.stringify({
        ...
        level: 'error',
        context: { page: 'multi-sig', digest: error.digest },
      }),
    }).catch(() => {})
  }, [error])
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#050507] via-[#0a0a0f] to-[#050507] text-white flex items-center justify-center">
      <div className="max-w-md mx-auto text-center px-6">
        ...
        <h1 className="text-2xl font-bold mb-3 bg-gradient-to-r from-[#00e5ff] to-[#00ff87] bg-clip-text text-transparent">Recovery System Error</h1>
        <p className="text-white/40 text-sm mb-8">
          The multi-sig recovery system encountered an error. This has been logged.
        </p>
        <div className="flex gap-3 justify-center">
          <button
            onClick={reset}
            className="px-6 py-3 bg-gradient-to-r from-[#00e5ff] to-[#00ff87] rounded-xl font-semibold text-sm text-black hover:shadow-[0_0_30px_rgba(0,229,255,0.2)] transition-all"
          >
            Retry
          </button>
          <a
